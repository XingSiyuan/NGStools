...
getopt('ap', \%opts);
my $archive = $opts{a};
my $archivepath = $opts{p};

my $db = DBI->connect("dbi:mysql:$database:$server", $user, $passwd);
my $basedir=$archivepath;
my $query = "select archive_name,lane_names_orig,md5sum_gzip from ABGSAschema_main where archive_name = '".$archive."'";
...
sub update {
	...
	#$sql -> execute($size);
        ...
        my $sql = $db ->prepare($query);
        $sql -> execute($md5);
        $query = "update filesize set filesize.filesize = ? where lane_names_orig = ('$file');";
